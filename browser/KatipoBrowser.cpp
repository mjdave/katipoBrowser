...
void KatipoBrowser::init()
{
    ...
    rootTable->getTable("file")->setFunction("getSavePath", [rootTable](TuiTable* args, TuiRef* existingResult, TuiDebugInfo* callingDebugInfo) -> TuiRef* {
        ...
        {
            return new TuiString(getSavePath(args->arrayObjects[0]->getStringValue()));
        }
        return new TuiString(getSavePath());
    });
    ...
    rootTable->setFunction("require", [rootTable](TuiTable* args, TuiRef* existingResult, TuiDebugInfo* callingDebugInfo) -> TuiRef* {
        if(args->arrayObjects.size() > 0 && args->arrayObjects[0]->type() == Tui_ref_type_STRING)
        {
            return TuiRef::load(getResourcePath(args->arrayObjects[0]->getStringValue()), rootTable); //override require to use our resource path, not tui's
        }
        return nullptr;
    });
    ...
    TuiTable* sceneTable = (TuiTable*)TuiRef::load(getResourcePath("scripts/scene.tui"), rootTable);
   ...
   // mainView = new MJView(MainController::getInstance()->mainMJView);
    
    mainView = MJView::loadUnknownViewFromTable(sceneTable->getTable("mainView"), MainController::getInstance()->mainMJView, true);
    //mainView->loadFromTable(sceneTable->getTable("mainView"), true);
    
   /* sceneTable->setFunction("getView", [this](TuiTable* args, TuiTable* state, TuiRef* existingResult, TuiDebugInfo* callingDebugInfo) -> TuiRef* {
        if(args->arrayObjects.size() >= 1)
        {
            TuiRef* viewNameRef = args->arrayObjects[0];
            if(viewNameRef->type() == Tui_ref_type_STRING)
    ...
    scriptState = TuiRef::runScriptFile(getResourcePath("scripts/code.tui"), rootTable);
