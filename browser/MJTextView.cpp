...
void MJTextView::updateDimensions()
{
	if(font)
	{
        ...
        if(wrapClampCharIndex > -1 && stateTable->objectsByStringKey.count("text") != 0)
        {
            ...
            {
                ((TuiString*)stateTable->objectsByStringKey["text"])->value = "";
            }
            else
            {
...
void MJTextView::updateBuffer(GCommandBuffer* commandBuffer)
{
    ...
    if(font && !text.empty())
    {
        ...
        nextVertices = font->print(text, textAlignment, wrapWidth * scaleToUse, &enclosingRect, 1.0 / textRenderScale, wrapClamp, nullptr, preventWordSeparation).fontVerts;
...
void MJTextView::loadFromTable(TuiTable* table, bool needsToLayoutSubviews, TuiTable* rootTableOrNil)
{
    ...
    if(table->hasKey("preventWordSeparation"))
    {
        stateTable->setBool("preventWordSeparation", table->getBool("preventWordSeparation"));
    }
...
void MJTextView::tableKeyChanged(const std::string& key, TuiRef* value)
{
    ...
    else if(key == "preventWordSeparation")
    {
        preventWordSeparation = stateTable->getBool("preventWordSeparation");
    }
